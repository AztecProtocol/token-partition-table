...
import AttestorContractArtifactJson from './attestor-Attestor.json' assert { type: 'json' };
...
export class AttestorContract extends ContractBase {
  ...
  public methods!: {
    ...
    /** request_attestation(partition_table: struct, proofs: array, blacklist_root: field) */
    request_attestation: ((partition_table: { shield_ids: { len: (bigint | number), elems: FieldLike[] }, attestations: { len: (bigint | number), elems: FieldLike[] }, max_block_number: (bigint | number), is_table_cleared: boolean }, proofs: FieldLike[], blacklist_root: FieldLike) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;
