...
describe('e2e_token_contract', () => {
    ...
    describe('Private', () => {
      ...
      describe('Mint flow', () => {
        ...
        it('redeem as recipient', async () => {
          await addPendingShieldNoteToPXE(0, amount, blockNumber, secretHash, txHash);
          const numShields = await asset.methods.num_shields().view();
          const txClaim = asset.methods
            .redeem_shield(accounts[0].address, amount, blockNumber, secret, numShields)
            .send();
          // docs:start:debug
      ...
      describe('failure cases', () => {
        it('try to redeem as recipient (double-spend) [REVERTS]', async () => {
          await expect(addPendingShieldNoteToPXE(0, amount, blockNumber, secretHash, txHash)).rejects.toThrow(
            'The note has been destroyed.',
          );
          const numShields = await asset.methods.num_shields().view();
          await expect(
            asset.methods.redeem_shield(accounts[0].address, amount, blockNumber, secret, numShields).simulate(),
  ...
  describe('Shielding (shield + redeem_shield)', () => {
    ...
    it('on behalf of self', async () => {
      ...
      // Redeem it
      await addPendingShieldNoteToPXE(0, amount, blockNumber, secretHash, receipt.txHash);
      const numShields = await asset.methods.num_shields().view();
      await asset.methods.redeem_shield(accounts[0].address, amount, blockNumber, secret, numShields).send().wait();
  ...
  describe('Attestations', () => {
    ...
    describe('No transfers', () => {
      beforeEach(async () => {
        ...
        await asset.methods.mint_public(accounts[0], amount).send().wait();
        tokenSim.mintPublic(accounts[0].address, amount);
      ...
      it('Request attestation', async () => {
        ...
        await asset.methods
          .request_attestation(accounts[0].address, attestor.address, 0, proofs.flat(), root, 0)
          .send()
          .wait();
    ...
    describe('Partial transfers', () => {
      ...
      it('Attestation transfer', async () => {
        ...
        await asset
          .withWallet(wallet1)
          .methods.request_attestation(account1, attestor.address, 0, proofs.flat(), root, 0)
          .send()
          .wait();
      ...
      it('Deposit ID transfer', async () => {
        ...
        // const tx4 = asset.withWallet(wallet1).methods.request_attestation(account1, attestor.address, 0, proof2, root2, 0).send();

        // Request attestation should fail i.e. not add any attestation
        const proofs = await attestorSim.getSiblingPaths(asset.address, shieldIds);
        await asset
          .withWallet(wallet2)
          .methods.request_attestation(account2, attestor.address, 0, proofs.flat(), root2, 0)
          .send()
          .wait();
